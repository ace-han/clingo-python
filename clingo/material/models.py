    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['pickup', 'start_sec', 'end_sec'], name='unique_material_preprocessing'
            )
        ]

class ProofReading(models.Model):
    processing = models.ForeignKey(RawPickup, on_delete=models.CASCADE)
    source_language = models.CharField(max_length=16, choices=settings.LANGUAGES, default=settings.LANGUAGE_CODE)
    target_language = models.CharField(max_length=16, choices=settings.LANGUAGES, default=settings.LANGUAGE_CODE)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['processing', 'source_language', 'target_language'], name='unique_material_proofreading'
            )
        ]
...
class VideoClip(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['proofreading', 'title',], name='unique_material_videoclip'
            )
        ]
